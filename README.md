...
- 按室内空间管理点位（名称、数量、单位、单项备注），支持**自定义分类**（图标/颜色/重命名/删除）
- 每个空间支持单独备注；右侧栏支持多条全局备注
- **全屋汇总清单**：跨空间同名项目数量合计，可直接用于采购 / 电工交底，支持打印
- **全局搜索**：跨空间按名称/备注搜索，点击结果直接跳转高亮
- **快速复用**：复制整个空间、从其他空间按分类复制项目、输入时自动联想已有名称
- 一键导出 Excel（.xlsx，含点位总表 / 明细清单 / 汇总清单 / 全局备注四个工作表）
- **删除可撤销**：删除空间 / 项目 / 分类 / 备注后，提示条内点「撤销」即可恢复
- 拖拽排序：侧栏空间、分类内项目均可拖动调整顺序
- 数据自动保存在本机浏览器（localStorage），支持 JSON 备份 / 导入迁移
- **PWA**：手机端「添加到主屏幕」后可全屏使用，离线也能打开（ServiceWorker 缓存）
